...
func (c chatDb) GetByUser(userID int) ([]domain.Chat, error) {
	...
	err := c.db.Select(&chats, `select chats.id, chats.title, chats.model, chats.type from chats left join messages on messages.chat_id = chats.id where user_id = ? order by messages.created_at desc`, userID)
	return chats, err
}
